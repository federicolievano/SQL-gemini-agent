...
st.markdown("Ask questions in natural language and get SQL results from your MySQL database")
        ...
        if user_question:
            with st.spinner("Generating SQL query..."):
                try:
                    ...
                    with st.spinner("🔄 Generating SQL (this may take 10-30 seconds)..."):
                        sql_query = llm.invoke(sql_prompt.format(question=user_question))
...
if 'results' in st.session_state and st.session_state.results is not None:
    ...
    with col_info3:
        st.metric("Memory Usage", f"{st.session_state.results.memory_usage(deep=True).sum() / 1024:.1f} KB")
